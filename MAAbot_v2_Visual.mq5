...
// Objeto trade global (usado pelos módulos via extern)
CTrade trade;
...
void OnTick() {
   ...
   if(MG_Mode == MG_GRID) {
      if(GridTPHit(+1)) { CloseBasket(+1); GridReset(+1); g_lastAction = "Grid TP Buy"; g_lastActionTime = now; }
      if(GridTPHit(-1)) { CloseBasket(-1); GridReset(-1); g_lastAction = "Grid TP Sell"; g_lastActionTime = now; }
      if(GridStopHit(+1)) { CloseBasket(+1); GridReset(+1); g_lastAction = "Grid SL Buy"; g_lastActionTime = now; }
      if(GridStopHit(-1)) { CloseBasket(-1); GridReset(-1); g_lastAction = "Grid SL Sell"; g_lastActionTime = now; }
   }
   
   HedgeRecoveryCheck();
   
   if(MG_Mode == MG_GRID) {
      if(HasBasket(+1)) GridTryAdd(S, +1);
      if(HasBasket(-1)) GridTryAdd(S, -1);
   }
   
   if(CloseOnFlip && MG_Mode != MG_GRID) { 
      if(pS >= thrS && pS > pL) { CloseBasket(+1); g_lastAction = "Flip Buy"; g_lastActionTime = now; }
      if(pL >= thrL && pL > pS) { CloseBasket(-1); g_lastAction = "Flip Sell"; g_lastActionTime = now; }
   }
   else if(CloseOnFlip && MG_Mode == MG_GRID && !MG_Grid_DisableFlipClose) {
      if(pS >= thrS && pS > pL) { CloseBasket(+1); GridReset(+1); g_lastAction = "Flip Buy Grid"; g_lastActionTime = now; }
      if(pL >= thrL && pL > pS) { CloseBasket(-1); GridReset(-1); g_lastAction = "Flip Sell Grid"; g_lastActionTime = now; } 
   }
   ...
   if(isTradingHours) {
      ...
      if(Hedge_Enable && Hedge_AllowDoubleStart) { 
         if(wantBuy && !HasBasket(+1)) TryOpen(+1, S, pL, thrL, now); 
         if(wantSell && !HasBasket(-1)) TryOpen(-1, S, pS, thrS, now); 
      }
      else {
         if(wantBuy && wantSell) {
            if(pL >= pS && !HasBasket(+1)) TryOpen(+1, S, pL, thrL, now);
            else if(pS > pL && !HasBasket(-1)) TryOpen(-1, S, pS, thrS, now);
         }
         else if(wantBuy && !HasBasket(+1)) TryOpen(+1, S, pL, thrL, now);
         else if(wantSell && !HasBasket(-1)) TryOpen(-1, S, pS, thrS, now);
      }
      
      if(Hedge_Enable) {
         if(Hedge_OpenOnOppSignal) { 
            if(HasBasket(+1) && wantSell && !HasBasket(-1)) TryOpen(-1, S, pS, thrS, now); 
            if(HasBasket(-1) && wantBuy && !HasBasket(+1)) TryOpen(+1, S, pL, thrL, now); 
         }
         if(Hedge_OpenOnAdverse) { 
            if(HasBasket(+1) && !HasBasket(-1) && AdverseTrigger(+1)) TryOpen(-1, S, 1.0, 0.0, now);
            if(HasBasket(-1) && !HasBasket(+1) && AdverseTrigger(-1)) TryOpen(+1, S, 1.0, 0.0, now); 
         }
      }
   }
